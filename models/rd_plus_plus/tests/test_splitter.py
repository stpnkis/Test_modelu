...
class TestCreateSplit(_SplitterTestBase):
    ...
    def test_value_error_too_few_train(self) -> None:
        """Requesting fewer than MIN_TRAIN_IMAGES (10) raises ValueError."""
        out = Path(self.tmp) / "split"
        with self.assertRaises(ValueError):
            create_split(str(self.dataset_dir), str(out), n_train=5, seed=42)
        with self.assertRaises(ValueError):
            create_split(str(self.dataset_dir), str(out), n_train=9, seed=42)
        # Boundary: exactly 10 should NOT raise
