"""
Dataset Splitter for RD++ Experiment
======================================
...
**Identical split logic to SimpleNet / PatchCore** — deterministic with the
same seed so that all models are evaluated on exactly the same test set.

Guard: n_train must be ≥ MIN_TRAIN_IMAGES (10) for reliable results.
...
def create_split(
    ...
    if n_train < MIN_TRAIN_IMAGES:
        msg = (
            ...
            f"  an unreliable feature reconstruction baseline.\n"
            f"\n"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Split casting dataset for RD++ experiment"
    )
    parser.add_argument(
