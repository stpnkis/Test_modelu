...
are computed identically to PatchCore's evaluate.py on synthetic data.
...
def _compute_metrics(labels: np.ndarray, scores: np.ndarray) -> dict:
    """Replicates the metric block from evaluate.py (and PatchCore's evaluate.py).
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_all_positive(self) -> None:
        ...
        # roc_auc_score technically needs both classes; skip AUROC check
        # We only verify no exception and recall == 1.
        fpr, tpr, thresholds = roc_curve(labels, scores)
    ...
    def test_single_class_is_undefined(self) -> None:
        ...
        try:
            ...
            # Newer sklearn may return NaN instead of raising
